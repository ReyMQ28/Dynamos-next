...
import { enviarCorreo } from "../../InitialProjects/dynamos-movimiento/src/config/enviarCorreo";
    ...
    <div className="flex md:flex-row md:mb-10 sm:mb-5 md:w-full sm:flex-wrap sm:justify-center sm:items-center sm:w-10/12 md:mt-5">
      ...
      {red.instagram && (
        <Link href={red.instagram} target="_blank">
          <BsInstagram className="md:mx-5 sm:mx-2 md:text-3xl sm:text-base cursor-pointer hover:text-pink-600 " />
        </Link>
      )}
      {red.youtube && (
      ...
      {red.correo && (
        <Link href={red.correo} onClick={enviarCorreo}>
          <BsFillEnvelopeAtFill className="md:mx-5 sm:mx-2 md:text-3xl sm:text-base cursor-pointer hover:text-black" />
        </Link>
      )}
    </div>
  );
